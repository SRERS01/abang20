...
async function runMassScanner() {
    ...
    for (let baseScope of targetList) {
        ...
        // === PHASE 1: AUTOMATED SPIDER AND ENDPOINT SCRAPING ===
        ...
        try {
            ...
            // Extract links and input definitions directly from the layout structure
            const discoveredInfo = await crawlPage.evaluate((domain) => {
                const links = [];
                const formsAndParams = [];

                // 1. Grab all anchor tags on the home page
            ...
            // Filter out foreign external links to keep the scanner tightly in-scope
            ...
            // Convert our crawled endpoints into fuzzable structures mapping parameters
            crawledEndpoints.forEach(endpoint => {
                const parameters = discoveredInfo.formsAndParams.length > 0 ? discoveredInfo.formsAndParams : ['search', 'q', 'id'];
                endpointsToFuzz.push({ endpoint, parameters });
            });
