...
if (args.length === 0) {
    ...
    console.log("💡 Usage (Single URL) : node xss_final_engine.js https://example.com");
    console.log("💡 Usage (Mass File) : node xss_final_engine.js targets.txt");
    process.exit(1);
}
...
// Function to log vulnerabilities to disk
function logVulnerability(issue) {
    ...
    console.log(`   💾 [SAVED]: Finding archived to loot/findings.json`);
}
    ...
    for (let baseUrl of targetList) {
        ...
        for (let payload of payloads) {
            ...
            for (let target of testUrls) {
                ...
                // 1. Dialog Listener: Proves Execution
                attackPage.on('dialog', async dialog => {
                    if (dialog.message() || dialog.type() === 'alert') {
                        ...
                        // Output finding cleanly to disk
                        logVulnerability({
                            vulnerability_type: "DOM-XSS (Execution Verified)",
                            target_scope: baseUrl,
                            vector_type: target.type,
                            exploit_url: target.url,
                            payload: payload,
                    ...
                    try {
                        ...
                        if (contentType.includes('application/json') || contentType.includes('text/plain') || contentType.includes('text/html')) {
                            ...
                            if (responseBody.includes(payload)) {
                                ...
                                logVulnerability({
                                    vulnerability_type: "API Response Reflection (Potential)",
                                    target_scope: baseUrl,
                                    vector_type: "AJAX Stream Intercept",
                                    exploit_url: target.url,
                                    payload: payload,
                                    reflected_endpoint: response.url()
                                });
                            }
                        }
                    } catch (err) {
