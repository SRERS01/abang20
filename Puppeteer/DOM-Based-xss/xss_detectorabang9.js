...
async function runMassScanner() {
    ...
    for (let baseScope of targetList) {
        ...
        try {
            targetDomain = new URL(baseScope).hostname;
        } catch (e) {
            console.log(`[-] Skipping invalid URL structure: ${baseScope}`);
            continue;
        }
        ...
        // [📡 SPA Network Interceptor]: Strictly limited to in-scope domain targets
        ...
        try {
            ...
            // Allow client-side framework loops a 3-second window to finish executing background network requests
            ...
            // Filter static HTML link hrefs to ensure they fit our scope domain
            discoveredInfo.links.forEach(link => {
                try {
                    const linkUrl = new URL(link);
                    if (linkUrl.hostname === targetDomain || linkUrl.hostname.endsWith('.' + targetDomain)) {
                        crawledEndpoints.add(linkUrl.origin + linkUrl.pathname);
                    }
                } catch (e) {}
            });

            // Fallback parameter keys if the target application is completely empty
            const trackedParams = discoveredInfo.formsAndParams.length > 0 ? discoveredInfo.formsAndParams : ['search', 'q', 'id'];

            // Map all target inputs into structural object definitions
        ...
        for (const scopeDetails of endpointsToFuzz) {
            ...
            for (let payload of payloads) {
                ...
                // Vector A: Automated Query Combinations (Safe against pre-existing URL arguments)
                scopeDetails.parameters.forEach(param => {
                    try {
                        const parsedFuzzUrl = new URL(currentUrl);
                        parsedFuzzUrl.searchParams.set(param, payload);
                        
                        testUrls.push({
                            url: parsedFuzzUrl.toString(),
                            type: `QUERY (${param})`
                        });
                    } catch (e) {
                        testUrls.push({
                ...
                try {
                    const parsedHashUrl = new URL(currentUrl);
                    parsedHashUrl.hash = encodeURIComponent(payload);
                    testUrls.push({ url: parsedHashUrl.toString(), type: 'HASH (#)' });
                } catch(e) {
                ...
                try {
                    const parsedPathUrl = new URL(currentUrl);
                    parsedPathUrl.pathname = path.join(parsedPathUrl.pathname, encodeURIComponent(payload));
                    testUrls.push({ url: parsedPathUrl.toString(), type: 'PATH-ROUTER' });
                } catch(e) {
                ...
                for (let target of testUrls) {
                    ...
                    await attackPage.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, with Gecko) Chrome/120.0.0.0 Safari/537.36');
                    ...
                    // API Interception within the attack execution window
                    attackPage.on('response', async response => {
                        try {
const contentType = response.headers()['content-type'] || '';if (contentType.includes('application/json') || contentType.includes('text/html')) {const body = await response.text();if (body.includes(payload)) {console.log(   [🚨 API Data Reflection]: Injected code tracked returning through server endpoint: ${response.url()});logVulnerability({ target_scope: baseScope, exploit_url: target.url, payload: payload, API_reflection: response.url() });}}} catch (e) {}});try {console.log(   [→] Testing ${target.type}: ${target.url});await attackPage.goto(target.url, { waitUntil: 'domcontentloaded', timeout: 25000 });await new Promise(resolve => setTimeout(resolve, 2000));} catch (error) {console.log(   [-] Timeout handling vector invocation.);} finally {await attackPage.close();}}}}}await browser.close();console.log(\n[+] Dynamic Master Scan Campaign Finished.);}runMassScanner();
